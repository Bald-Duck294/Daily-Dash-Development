...
export default function ClientDashboard() {
  ...
  // Logic to prevent queries from running if user lacks permission
  ...
  // 1. Counts (Assuming general counts are viewable if they can access the dashboard)
  const { data: statsData = {
    ...
    totalRepairs: 0,
    totalCleaners: 0,
  }, isLoading: isCountsLoading } = useDashboardCounts(companyId, today);

  // 2. Top Locations
  const { data: topLocations = [], isLoading: isTopLocLoading } = 
    useDashboardTopLocations(activeCompanyIdForLocations, 5, today);

  // 3. Activities
  const { data: recentActivities = [], isLoading: isActivitiesLoading } = 
    useDashboardActivities(activeCompanyIdForReviews, 10, today);
  ...
  const { data: cleanerGraphData = { data: [], today_completed_tasks: 0 }, isLoading: isCleanerLoading } = 
    useCleanerPerformance(activeCompanyIdForReviews);

  // Combine loading states
  const isLoading = isCountsLoading || isTopLocLoading || isActivitiesLoading || isWashroomLoading || isCleanerLoading;
